...
VAULT_READ="vault read --format=json"
JQ="jq -r .data.value"
...
DO_TOKEN=$( $VAULT_READ secret/terraform_digital_ocean_api_key | $JQ )
echo "Getting buildkite agent token..."
BUILDKITE_AGENT_TOKEN=$( $VAULT_READ secret/buildkite_agent_token | $JQ )
echo "Getting registry google cloud storage token..."
REGISTRY_GCS_TOKEN=$( $VAULT_READ secret/registry_gcs_key | $JQ )
echo "Getting registry htpasswd"
REGISTRY_HTPASSWD=$( $VAULT_READ secret/registry_htpassw | $JQ )
echo "Getting dnsimple api token..."
DNSIMPLE_TOKEN=$( $VAULT_READ secret/dnsimple_token | $JQ )
echo "Getting dnsimple account id..."
DNSIMPLE_ACCOUNT=$( $VAULT_READ secret/dnsimple_account | $JQ )
